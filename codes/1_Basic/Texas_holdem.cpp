...
struct cards {
  ...
  void reset() {
    v.clear(), FILL(suit_count, 0), hands = -1;
  }
  ...
  void ready() {
    int Straight = 0,
        Flush = (*max_element(
                   suit_count, suit_count + 4) == 5);
    sort(ALL(v), [](ii a, ii b) { return a > b; });
    if (SZ(v) == 5 && v[0].Y == v[1].Y + 1 &&
      v[1].Y == v[2].Y + 1 && v[2].Y == v[3].Y + 1 &&
      v[3].Y == v[4].Y + 1)
      Straight = 1;
    else if (SZ(v) == 5 && v[0].Y == 12 &&
      v[1].Y == 3 && v[2].Y == 2 && v[3].Y == 1 &&
      v[4].Y == 0)
      v[0].Y = 3, v[1].Y = 2, v[2].Y = 1, v[1].Y = 0,
      v[0].Y = -1, Straight = 1;
