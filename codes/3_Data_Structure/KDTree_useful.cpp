...
private:
  ...
  struct __cmp {
    ...
    inline bool operator()(
      const node *x, const node *y) const {
  ...
  node *build(int k, int l, int r) {
    ...
    int mid = (l + r) / 2;
    cmp.sort_id = k;
    std::nth_element(A.begin() + l, A.begin() + mid,
      A.begin() + r + 1, cmp);
    node *ret = A[mid];
  ...
  bool insert(
    node *&u, int k, const point &x, int dep) {
    if (!u) {
      u = new node(x);
      return dep <= 0;
    }
  ...
  node *findmin(node *o, int k) {
    if (!o) return 0;
    if (cmp.sort_id == k)
      return o->l ? findmin(o->l, (k + 1) % kd) : o;
  ...
  void nearest(
    node *u, int k, const point &x, T *h, T &mndist) {
...
public:
  ...
  inline const std::vector<point> &range(
    const point &mi, const point &ma) {
    in_range.clear();
    range(root, 0, mi, ma);
