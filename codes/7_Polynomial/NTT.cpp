// 1:2615053605667*(2^18)+1,3 | 2:15*(2^27)+1,31
// 3:479*(2^21)+1,3           | 4:7*17*(2^23)+1,3
// 5:3*3*211*(2^19)+1,5       | 6:25*(2^22)+1,3
    ...
    for(int step=2,id=1;step<=N;step<<=1,++id){
      ...
      for(int i=0;i<mh;++i){
        for(int j=i;j<N;j+=step){
          u=out[j],t=wi*out[j+mh]%P; out[j]=u+t; out[j+mh]=u-t;
          if(out[j]>=P)out[j]-=P; if(out[j+mh]<0)out[j+mh]+=P;
        }
        wi=wi*wn%P;
      }
    }
