struct convex_hull_3D {
  struct Face {
    int a, b, c;
    Face(int ta, int tb, int tc)
      : a(ta), b(tb), c(tc) {}
  }; // return the faces with pt indexes
  vector<Face> res;
  vector<Point> P;
  convex_hull_3D(const vector<Point> &_P)
    : res(), P(_P) {
    ...
    swap(P[1], *find_if(ALL(P), [&](auto p) {
      return sign(abs2(P[0] - p)) != 0;
    }));
    swap(P[2], *find_if(ALL(P), [&](auto p) {
      return sign(abs2(cross3(p, P[0], P[1]))) != 0;
    }));
  ...
  bool same(Face s, Face t) {
    if (sign(volume(P[s.a], P[s.b], P[s.c], P[t.a])) !=
      0)
      return 0;
    if (sign(volume(P[s.a], P[s.b], P[s.c], P[t.b])) !=
      0)
      return 0;
    if (sign(volume(P[s.a], P[s.b], P[s.c], P[t.c])) !=
      0)
      return 0;
    return 1;
  }
