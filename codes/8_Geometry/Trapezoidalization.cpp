template <class T> struct SweepLine {
  struct cmp {
  ...
  vector<typename multiset<pair<T, int>>::iterator>
    eits;
  bool slope_cmp(int a, int b) const {
    assert(a != -1);
    if (b == -1) return 0;
    return sign(cross(base[a].Y - base[a].X,
             base[b].Y - base[b].X)) < 0;
  }
  ...
  void erase(int idx) {
    ...
    its[idx] = sweep.end();
    if (p != sweep.begin()) update_event(*prev(p));
  }
  void update_event(int idx) {
    ...
    if (nxt == sweep.end() || !slope_cmp(idx, *nxt))
      return;
    auto t = intersect(base[idx].X, base[idx].Y,
      base[*nxt].X, base[*nxt].Y)
               .X;
    if (t + eps < curTime ||
      t >= min(base[idx].Y.X, base[*nxt].Y.X))
      return;
  ...
  SweepLine(T t, T e, vector<Line> vec)
    ...
    for (int i = 0; i < SZ(base); ++i) {
      auto &[p, q] = base[i];
      if (p > q) swap(p, q);
      if (p.X <= curTime && curTime <= q.X) insert(i);
      else if (curTime < p.X) event.emplace(p.X, i);
    }
  }
