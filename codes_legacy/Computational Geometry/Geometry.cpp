...
template <typename T> struct line {
  ...
  T ori(const point<T> &p)
    const { // 點和有向直線的關係，>0左邊、=0在線上<0右邊
    return (p2 - p1).cross(p - p1);
  }
  T btw(
    const point<T> &p) const { // 點投影落在線段上<=0
    return (p1 - p).dot(p2 - p);
  }
  bool point_on_segment(
    const point<T> &p) const { // 點是否在線段上
    return ori(p) == 0 && btw(p) <= 0;
  }
  T dis2(const point<T> &p, bool is_segment = 0)
    const { // 點跟直線/線段的距離平方
  ...
  int line_intersect(const line &l)
    const { // 直線相交情況，-1無限多點、1交於一點、0不相交
  ...
  point<T> line_intersection(
    const line &l) const { /*直線交點*/
    point<T> a = p2 - p1, b = l.p2 - l.p1,
             s = l.p1 - p1;
    // if(a.cross(b)==0)return INF;
    return p1 + a * (s.cross(b) / a.cross(b));
  }
  point<T> seg_intersection(
    const line &l) const { // 線段交點
...
template <typename T> struct polygon {
  ...
  char ahas(const point<T> &t)
    const { // 點是否在簡單多邊形內，是的話回傳1、在邊上回傳-1、否則回傳0
    bool c = 0;
    for (int i = 0, j = p.size() - 1; i < p.size();
         j = i++)
  ...
  char point_in_convex(const point<T> &x) const {
    ...
    while (l <=
      r) { // 點是否在凸多邊形內，是的話回傳1、在邊上回傳-1、否則回傳0
      int mid = (l + r) / 2;
  ...
  polygon cut(const line<T> &l)
    const { // 凸包對直線切割，得到直線l左側的凸包
    polygon ans;
    for (int n = p.size(), i = n - 1, j = 0; j < n;
         i = j++) {
      if (l.ori(p[i]) >= 0) {
        ans.p.push_back(p[i]);
        if (l.ori(p[j]) < 0)
          ans.p.push_back(
            l.line_intersection(line<T>(p[i], p[j])));
      } else if (l.ori(p[j]) > 0)
        ans.p.push_back(
          l.line_intersection(line<T>(p[i], p[j])));
    }
  ...
  static bool monotone_chain_cmp(const point<T> &a,
  ...
  void monotone_chain(vector<point<T>> &s) { // 凸包
    ...
    for (size_t i = 0; i < s.size(); ++i) {
      while (m >= 2 &&
        (p[m - 1] - p[m - 2]).cross(s[i] - p[m - 2]) <=
          0)
        --m;
      p[m++] = s[i];
    }
    for (int i = s.size() - 2, t = m + 1; i >= 0;
         --i) {
      while (m >= t &&
        (p[m - 1] - p[m - 2]).cross(s[i] - p[m - 2]) <=
          0)
  ...
  T min_cover_rectangle() { // 最小覆蓋矩形
    ...
    for (int i = 0; i < n; i++) {
      ...
      T tmp = now.cross(p[t] - p[i]) *
        (now.dot(p[r] - p[i]) - now.dot(p[l] - p[i])) /
        d;
      ans = min(ans, tmp);
    }
    return p.pop_back(), ans;
  ...
  static bool angle_cmp(
    const line<T> &A, const line<T> &B) {
    point<T> a = A.p2 - A.p1, b = B.p2 - B.p1;
    return sign(a) < sign(b) ||
      (sign(a) == sign(b) && a.cross(b) > 0);
  }
  int halfplane_intersection(
    vector<line<T>> &s) { // 半平面交
    sort(s.begin(), s.end(),
      angle_cmp); // 線段左側為該線段半平面
    int L, R, n = s.size();
    vector<point<T>> px(n);
    vector<line<T>> q(n);
    q[L = R = 0] = s[0];
...
template <typename T> struct triangle {
  point<T> a, b, c;
  triangle() {}
  triangle(const point<T> &a, const point<T> &b,
    const point<T> &c)
    : a(a), b(b), c(c) {}
  T area() const {
    T t = (b - a).cross(c - a) / 2;
  ...
  point<T> circumcenter() const { // 外心
    ...
    u.p1 = (a + b) / 2;
    u.p2 =
      point<T>(u.p1.x - a.y + b.y, u.p1.y + a.x - b.x);
    v.p1 = (a + c) / 2;
    v.p2 =
      point<T>(v.p1.x - a.y + c.y, v.p1.y + a.x - c.x);
...
template <typename T> struct line3D {
  ...
  line3D(const point3D<T> &p1, const point3D<T> &p2)
    : p1(p1), p2(p2) {}
  T dis2(const point3D<T> &p, bool is_segment = 0)
    const { // 點跟直線/線段的距離平方
  ...
  pair<point3D<T>, point3D<T>> closest_pair(
    ...
    T tmp = N.dot(ab),
      ans = tmp * tmp / N.abs2(); // 最近點對距離
    point3D<T> d1 = p2 - p1, d2 = l.p2 - l.p1,
               D = d1.cross(d2), G = l.p1 - p1;
    T t1 = (G.cross(d2)).dot(D) / D.abs2();
    T t2 = (G.cross(d1)).dot(D) / D.abs2();
    return make_pair(p1 + d1 * t1, l.p1 + d2 * t2);
  }
  bool same_side(
    const point3D<T> &a, const point3D<T> &b) const {
    return (p2 - p1).cross(a - p1).dot(
             (p2 - p1).cross(b - p1)) > 0;
  }
};
template <typename T> struct plane {
  ...
  plane() {}
  plane(const point3D<T> &p0, const point3D<T> &n)
    : p0(p0), n(n) {}
  T dis2(
    const point3D<T> &p) const { // 點到平面距離的平方
    T tmp = (p - p0).dot(n);
...
template <typename T> struct triangle3D {
  point3D<T> a, b, c;
  triangle3D() {}
  triangle3D(const point3D<T> &a, const point3D<T> &b,
    const point3D<T> &c)
    : a(a), b(b), c(c) {}
  bool point_in(const point3D<T> &p)
    const { // 點在該平面上的投影在三角形中
    return line3D<T>(b, c).same_side(p, a) &&
      line3D<T>(a, c).same_side(p, b) &&
      line3D<T>(a, b).same_side(p, c);
  }
};
template <typename T> struct tetrahedron { // 四面體
  point3D<T> a, b, c, d;
  tetrahedron() {}
  tetrahedron(const point3D<T> &a, const point3D<T> &b,
...
template <typename T> struct convexhull3D {
  ...
  void build() {
    ...
    for (int i = 3, ftop = 1; i < n; ++i, ++ftop) {
      ...
      for (auto &f : ans) {
        ...
        fid[f.a][f.b] = fid[f.b][f.c] = fid[f.c][f.a] =
          ff;
      }
      for (auto &f : ans) {
        if (fid[f.a][f.b] > 0 &&
          fid[f.a][f.b] != fid[f.b][f.a])
          next.emplace_back(f.a, f.b, i);
        if (fid[f.b][f.c] > 0 &&
          fid[f.b][f.c] != fid[f.c][f.b])
          next.emplace_back(f.b, f.c, i);
        if (fid[f.c][f.a] > 0 &&
          fid[f.c][f.a] != fid[f.a][f.c])
