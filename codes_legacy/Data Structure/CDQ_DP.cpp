...
void CDQ(int l, int r) {
  ...
  int mid = (l + r) / 2;
  stable_partition(p + l, p + r + 1,
    [&](const node &d) { return d.id <= mid; });
  CDQ(l, mid);
  q.clear();
  for (int i = l, j; i <= mid; ++i) {
    while ((j = q.size()) > 1 &&
      Slope(q[j - 2], q[j - 1]) < Slope(q[j - 1], i))
      q.pop_back();
    q.push_back(i);
  }
  q.push_back(0);
  for (int i = mid + 1; i <= r; ++i) {
    while (q.size() > 1 && Slope(q[0], q[1]) > p[i].k)
      q.pop_front();
    DP[p[i].id] = max(DP[p[i].id],
      p[i].a * p[q[0]].x + p[i].b * p[q[0]].y);
  }
  CDQ(mid + 1, r);
