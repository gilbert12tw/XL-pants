...
struct DLX {
  ...
  void add(int r, int c) {
    ...
    if (H[r] < 0) H[r] = L[sz] = R[sz] = sz;
      ...
      R[sz] = R[H[r]], L[R[H[r]]] = sz, L[sz] = H[r],
      R[H[r]] = sz;
  }
#define DFOR(i, A, s)                                 \
  for (int i = A[s]; i != s; i = A[i])
  void remove(
    int c) { // 刪除第c行和所有當前覆蓋到第c行的列
    L[R[c]] = L[c],
    R[L[c]] = R
      [c]; // 這裡刪除第c行，若有些行不需要處理可以在開始時呼叫他
    DFOR(i, D, c) DFOR(j, R, i) {
      U[D[j]] = U[j], D[U[j]] = D[j], --S[col[j]];
    }
  }
  void restore(int
      c) { // 恢復第c行和所有當前覆蓋到第c行的列，remove的逆操作
    DFOR(i, U, c) DFOR(j, L, i) {
      ++S[col[j]], U[D[j]] = j, D[U[j]] = j;
    }
    L[R[c]] = c, R[L[c]] = c;
  }
  void remove2(int
      nd) { // 刪除nd所在的行當前所有點(包括虛擬節點)，只保留nd
    DFOR(i, D, nd) L[R[i]] = L[i], R[L[i]] = R[i];
  }
  void restore2(int
      nd) { // 刪除nd所在的行當前所有點，為remove2的逆操作
