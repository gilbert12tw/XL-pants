struct node { // kd樹代替高維線段樹
  ...
  point pid, mi, ma;
  int s, data;
  node(const point &p, int d)
    : l(0), r(0), pid(p), mi(p), ma(p), s(1), data(d),
      dmin(d), dmax(d) {}
  void up() {
    mi = ma = pid;
    s = 1;
...
// 檢查區間包含用的函數
bool range_include(
  node *o, const point &L, const point &R) {
  for (int i = 0; i < kd; ++i) {
...
// 單點修改，以單點改值為例
void update(
  node *u, const point &x, int data, int k = 0) {
  if (!u) return;
  u->down();
  if (u->pid == x) {
    u->data = data;
  ...
  cmp.sort_id = k;
  update(cmp(x, u->pid) ? u->l : u->r, x, data,
    (k + 1) % kd);
  u->up2();
}
// 區間修改
void update(
  node *o, const point &L, const point &R, int data) {
  if (!o) return;
  o->down();
