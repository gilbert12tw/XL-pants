...
struct Fringe {
  ...
  void merge(Fringe &o) {
    ...
    else make_onion_structure(o);
    if (o.FOPs.front().left.size())
      FOPs.push_front(o.FOPs.front());
  }
    ...
    if (FOPs.front().left.empty() ||
      (!FOPs.front().right.empty() &&
        FOPs.front().left.back() >
          FOPs.front().right.back())) {
      swap(FOPs.front().left, FOPs.front().right);
    }
  }
  void make_onion_structure(Fringe &o) {
    auto low = &FOPs.front().left,
         high = &FOPs.front().right;
    if (FOPs.front().left.front() >=
      FOPs.front().right.front())
      swap(low, high);
  ...
  void prune(int deep) {
    ...
    while (!FOPs.empty() && (left || right)) {
      ...
      if (!FOPs.empty())
        tie(left, right) = lr_condition(deep);
    }
  }
};
unique_ptr<Fringe> get_merged_fringe(
  deque<unique_ptr<Fringe>> &upper) {
  if (upper.empty()) return nullptr;
  sort(upper.begin(), upper.end(),
    [](auto &a, auto &b) { return *a < *b; });
  for (auto it = next(upper.begin());
       it != upper.end(); ++it)
...
void merge_fringes(
  ...
  if (mf) {
    mf->prune(deep);
    if (mf->FOPs.size())
      fringes.back().push_back(move(mf));
  }
}
struct Edge {
  int from, to;
...
struct Graph {
  ...
  void build() {
    ...
    edges.erase(
      unique(edges.begin(), edges.end()), edges.end());
    n = 0;
    for (auto &e : edges)
      n = max(n, max(e.from, e.to) + 1);
    neighbor.resize(n);
    for (auto &e : edges)
      neighbor[e.from].push_back(e.to);
  }
};
Graph g;
vector<int> Deeps;
...
bool dfs(int x, int parent = -1) {
  for (int y : g.neighbor[x]) {
    ...
    if (Deeps[y] < 0) { // tree edge
      ...
      fringes.back().push_back(
        make_unique<Fringe>(Deeps[y]));
    }
  }
  try {
    if (fringes.size() > 1)
      merge_fringes(fringes, Deeps[parent]);
