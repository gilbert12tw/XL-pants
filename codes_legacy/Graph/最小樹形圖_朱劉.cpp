template <typename T> struct zhu_liu {
  static const int MAXN = 110, MAXM = 10005;
  struct node {
    int u, v;
    T w, tag;
    node *l, *r;
    node(int u = 0, int v = 0, T w = 0)
      : u(u), v(v), w(w), tag(0), l(0), r(0) {}
    void down() {
      w += tag;
      if (l) l->tag += tag;
      if (r) r->tag += tag;
  ...
  T build(int root, int n) {
    ...
    for (int i = 1; i <= N; ++i) {
      ...
      if (find(E[i]->u, st) == find(i, st)) {
        ...
        st[find(i, st)] = find(E[i]->u, st), --all;
    }
