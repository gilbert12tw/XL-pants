...
struct CNF {
  ...
  int cost;
  CNF() {}
  CNF(int s, int x, int y, int c)
    : s(s), x(x), y(y), cost(c) {}
};
int state; // 規則數量
map<char, int>
  rule; // 每個字元對應到的規則，小寫字母為終端字符
vector<CNF> cnf;
void init() {
  state = 0;
  rule.clear();
...
void add_to_cnf(char s, const string &p, int cost) {
  ...
  for (auto c : p)
    if (rule.find(c) == rule.end()) rule[c] = state++;
  if (p.size() == 1) {
    ...
    cnf.push_back(CNF(
      left, rule[p[sz - 2]], rule[p[sz - 1]], cost));
  }
}
vector<long long> dp[MAXN][MAXN];
vector<bool>
  neg_INF[MAXN]
         [MAXN]; // 如果花費是負的可能會有無限小的情形
void relax(int l, int r, const CNF &c, long long cost,
  bool neg_c = 0) {
  if (!neg_INF[l][r][c.s] &&
