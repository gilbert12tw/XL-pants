2615053605667 * (2 ^ 18) + 1, 3 15 * (2 ^ 27) + 1,
  31 479 * (2 ^ 21) + 1, 3 7 * 17 * (2 ^ 23) + 1,
  3 3 * 3 * 211 * (2 ^ 19) + 1, 5 25 * (2 ^ 22) + 1,
  3 template <typename T, typename VT = vector<T>>
  struct NTT {
  const T P, G;
  NTT(T p = (1 << 23) * 7 * 17 + 1, T g = 3)
    : P(p), G(g) {}
  ...
  void ntt(bool is_inv, VT &in, VT &out, int N) {
    ...
    for (int step = 2, id = 1; step <= N;
         step <<= 1, ++id) {
      T wn = pow_mod(G, (P - 1) >> id, P), wi = 1, u,
        t;
      const int mh = step >> 1;
      for (int i = 0; i < mh; ++i) {
