...
bool g_test(
  const LL &g, const LL &p, const vector<LL> &v) {
...
LL Tonelli_Shanks(const LL &n, const LL &p) {
  ...
  if (n == 0) return 0;
  if (Legendre(n, p) != 1)
    while (1) { puts("SQRT ROOT does not exist"); }
  int S = 0;
  LL Q = p - 1;
  while (!(Q & 1)) {
    Q >>= 1;
...
template <typename T> T Euler(T n) {
  T ans = n;
  for (T i = 2; i * i <= n; ++i) {
    if (n % i == 0) {
      ans = ans / i * (i - 1);
...
template <typename T> T pow_mod(T n, T k, T m) {
  T ans = 1;
...
static void Pell(int n) {
  ...
  a0 = a1 =
    BigInteger.valueOf((int)Math.sqrt(1.0 * n));
